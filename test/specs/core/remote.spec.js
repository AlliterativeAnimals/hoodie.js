...
describe("Hoodie.Remote", function() {
  ...
  beforeEach(function() {
    ...
    this.triggerSpy = this.sandbox.spy(this.hoodie, 'trigger');
    this.oneSpy = this.sandbox.spy(this.hoodie, 'one');
    this.unbindSpy = this.sandbox.spy(this.hoodie, 'unbind');
    this.checkConnectionSpy = this.sandbox.spy(this.hoodie, 'checkConnection');

    this.timeoutSpy = this.sandbox.spy(window, 'setTimeout');

    this.requestDefer = this.hoodie.defer();

    this.dbStub = this.sandbox.stub(this.hoodie.account, 'db').returns('joe$example.com');

    this.remote = new Hoodie.Remote(this.hoodie);
    this.remote.name = 'remotetest';

    this.remoteStub = this.sandbox.stub(this.remote, 'request').returns(this.requestDefer.promise());

    this.connectSpy = this.sandbox.spy(Hoodie.Remote.prototype, 'connect');
  ...
  describe("constructor(@hoodie, options = {})", function() {
    ...
    //_when("connected: true passed", function() {
      ...
      //it("should start syncing", function() {
        //expect(this.connectSpy.called).to.be.ok();
      //});
